...
class DQNAgent:
    ...
    def __init__(self, env: GridWorld, config: Dict, device: torch.device):
        ...
        self.q_network = QNetwork(self.state_size, hidden_sizes, self.action_size).to(
            device
        )
        self.target_network = QNetwork(
            self.state_size, hidden_sizes, self.action_size
        ).to(device)
    ...
    def train_step(self):
        ...
        current_q = (
            self.q_network(state_batch).gather(1, action_batch.unsqueeze(1)).squeeze(1)
        )
