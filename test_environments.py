...
def test_environment():
    ...
    # Test initialization
    print(" Environment initialized: {env.rows}x{env.cols} grid")
    ...
    print(" Step executed: {state} -> {next_state}, reward={reward}, done={done}")
    ...
    print(" State conversion works: (1,2) <-> {state_idx}")
...
def test_network():
    ...
    print(
        " Network created with architecture: {input_size} -> {hidden_sizes} -> {output_size}"
    )
    ...
    print(" Forward pass works: input {dummy_input.shape} -> output {output.shape}")
...
def test_replay_buffer():
    ...
    print(" Buffer created with capacity: 100")

    # Add transitions
    for i in range(50):
        buffer.push((0, 0), i % 4, -0.04, (0, 1), False)

    print(" Added 50 transitions, buffer size: {len(buffer)}")

    # Sample batch
    batch = buffer.sample(32)
    assert len(batch) == 32, "Batch size mismatch"
    print(" Sampled batch of size: {len(batch)}")
    ...
    print(" Buffer capacity maintained at: {len(buffer)}")
...
def test_agent():
    ...
    if loss is not None:
        print(" Training step works: loss={loss:.4f}")

    # Test evaluation
    eval_mean, eval_std = agent.evaluate(n_episodes=10)
    print(" Evaluation works: mean={eval_mean:.3f}, std={eval_std:.3f}")
...
def quick_training_test():
    ...
    for episode in range(50):
        ...
        if (episode + 1) % 10 == 0:
            ...
            print(
                f"Episode {episode + 1}: avg_reward={avg_reward:.3f}, epsilon={agent.epsilon:.3f}"
            )
